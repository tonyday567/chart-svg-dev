...
#+begin_src haskell-ng :results output
(c,q) <- startChartServerWith (defaultSocketConfig & set #port 9166) (chartSocketPage Nothing)
:t c
#+end_src


#+RESULTS:
: Setting phasers gthoc is>t un...c  (::p orCth a9r1t6O6p)t i(ocntsr l->- c ItOo  qBuoiotl)
...
numbers = [20,25,15,5,35]
#+end_src

#+RESULTS:

#+begin_src haskell-ng :results output
d = BarData (pure numbers) words ["numbers"]
#+end_src

#+RESULTS:

#+begin_src haskell-ng :results output
c $ barChart (defaultBarOptions & set #barStacked Stacked & set #barOrientation Hori) (flipBarData d)
#+end_src
...
type BarOptions :: *
data BarOptions
  = BarOptions {barRectStyles :: [Chart.Style],
                barTextStyles :: [Chart.Style],
                outerGap :: Double,
                innerGap :: Double,
                textGap :: Double,
...
instance Eq BarOptions -- Defined in ‘Chart.Bar’
instance Generic BarOptions -- Defined in ‘Chart.Bar’
instance Show BarOptions -- Defined in ‘Chart.Bar’
type instance P.Rep BarOptions
  = D1
      (MetaData
         "BarOptions" "Chart.Bar" "chart-svg-0.8.2.1-inplace" False)
      (C1
         (MetaCons "BarOptions" PrefixI True)
         (((S1
              (MetaSel
                 (Just "barRectStyles")
...
example 1 : chroma from zero to one: a two second animation

#+begin_src haskell-ng :results output
f x = over chroma' (const (0.3*x))
glue (Committer c) <$|> animate 2 100 (\x -> Lens.transformOnOf Lens.template Lens.uniplate (f x) (unitExample & over #hudOptions (colourHudOptions (f x))))
#+end_src
...
#+begin_src haskell-ng :results output
f x = over hue' ((`modF` 360) . (+360*x))
glue (Committer c) <$|> animate 5 100 (\x -> Lens.transformOnOf Lens.template Lens.uniplate (f x) (unitExample & over #hudOptions (colourHudOptions (f x))))
#+end_src
...
#+begin_src haskell-ng :results output
f x = over lightness' (*x) >>> over chroma' (const (min 0.02 (0.2*x))) >>> over hue' ((`modF` 360) . (+360*x))
glue (Committer c) <$|> animate 2 100 (\x -> Lens.transformOnOf Lens.template Lens.uniplate (f x) (unitExample & over #hudOptions (colourHudOptions (f x))))
#+end_src
...
#+begin_src haskell-ng :results output
f x = over opac' (*x) >>> over chroma' (const (max 0.02 (0.2*x))) >>> over hue' ((`modF` 360) . (+360*x))
stamp x = TextChart defaultTextStyle [(fixed (Just 2) x, Point 0 0)]
glue (Committer c) <$|> animate 4 100 (\x -> Lens.transformOnOf Lens.template Lens.uniplate (f x) (unitExample & over #chartTree (<> (unnamed $ pure $ stamp x))))
#+end_src
...
stamp x = TextChart defaultTextStyle [(fixed (Just 2) x, Point 0 0)]
glue (Committer c) <$|> animate 4 100 (\x -> unitExample & over #chartTree (<> (unnamed $ pure $ stamp x)) & over (#chartTree % charts' % each % #chartData % rectData') (fmap (fmap (fmap (*x)))))
#+end_src
...
#+begin_src haskell-ng :results output
stamp x = TextChart defaultTextStyle [(fixed (Just 2) x, Point 0 0)]
glue (Committer c) <$|> animate 40 100 (\x -> unitExample & over #chartTree (<> (unnamed $ pure $ stamp x)) & over (#chartTree % charts' % each % #chartData % rectData') (fmap (fmap (fmap ((10 ** (10*x - 5))  *)))))
#+end_src
...
#+begin_src haskell-ng :results output
:t (\x -> unitExample & over (#chartTree % chart' % each % #chartData % rectData') (fmap (fmap (fmap (P.id)))))
#+end_src

#+begin_src haskell-ng :results output
:t \f x -> unitExample & over (#chartTree % charts' % each) f
#+end_src

#+RESULTS:
: <interactive>:1:51: error: [GHC-39999]
:     • No instance for ‘Each i0 Chart Chart a0 b0’
:         arising from a use of ‘each’
:     • In the second argument of ‘(%)’, namely ‘each’
:       In the first argument of ‘over’, namely
:         ‘(#chartTree % chart' % each)’
:       In the second argument of ‘(&)’, namely
:         ‘over (#chartTree % chart' % each) f’


#+begin_src haskell-ng :results output
:t unitExample & over #chartTree (<> (unnamed $ pure $ stamp 0.2)) & toListOf (#chartTree % rectData')
#+end_src


#+begin_src haskell-ng :results output
:t unitExample & over (#chartTree % charts' % each % #chartData % rectData') (fmap (fmap (fmap (0.5*))))
:t rectData'
#+end_src


#+RESULTS:
: unitExample & over (#chartTree % charts' % each % #chartData % rectData') (fmap (fmap (fmap (0.5*))))
:   :: ChartOptions
...
instance Generic ChartData -- Defined in ‘Chart.Primitive’
instance Eq ChartData -- Defined in ‘Chart.Primitive’
instance Show ChartData -- Defined in ‘Chart.Primitive’
type instance P.Rep ChartData
  = D1
      (MetaData
         "ChartData" "Chart.Primitive" "chart-svg-0.8.2.1-inplace" False)
      ((C1
          (MetaCons "RectData" PrefixI False)
          (S1
             (MetaSel
                Nothing NoSourceUnpackedness NoSourceStrictness DecidedLazy)
             (Rec0 [Rect Double]))
        :+: (C1
               (MetaCons "LineData" PrefixI False)
               (S1
                  (MetaSel
                     Nothing NoSourceUnpackedness NoSourceStrictness DecidedLazy)
                  (Rec0 [[Point Double]]))
             :+: C1
                   (MetaCons "GlyphData" PrefixI False)
                   (S1
                      (MetaSel
                         Nothing NoSourceUnpackedness NoSourceStrictness DecidedLazy)
                      (Rec0 [Point Double]))))
       :+: (C1
              (MetaCons "TextData" PrefixI False)
              (S1
                 (MetaSel
                    Nothing NoSourceUnpackedness NoSourceStrictness DecidedLazy)
                 (Rec0 [(Text, Point Double)]))
            :+: (C1
                   (MetaCons "PathData" PrefixI False)
                   (S1
                      (MetaSel
                         Nothing NoSourceUnpackedness NoSourceStrictness DecidedLazy)
                      (Rec0 [PathData Double]))
                 :+: C1
                       (MetaCons "BlankData" PrefixI False)
                       (S1
                          (MetaSel
                             Nothing NoSourceUnpackedness NoSourceStrictness DecidedLazy)
                          (Rec0 [Rect Double])))))
...
import Control.Lens qualified as Lens
import Data.Data.Lens qualified as Lens
display $ Lens.transformOnOf Lens.template Lens.uniplate (over chroma' (*0.4) >>> over opac' (*2.2)) lineExample
#+end_src
...
import Box
:t gapEffect
grain = grid OuterPos (Range 0 1) 100 :: [Double]
ani = qList $ fmap (\x -> Lens.transformOnOf Lens.template Lens.uniplate (over chroma' (*(0.4 + x)) >>> over opac' (*2.2)) lineExample) grain
...
: display :: ChartOptions -> IO Bool
: ani :: CoEmitter IO ChartOptions


#+begin_src haskell-ng :results output
:t Committer display
:t glue (Committer display)
:t ani
#+end_src

#+RESULTS:
: Committer display :: Committer IO ChartOptions
: glue (Committer display) :: Emitter IO ChartOptions -> IO ()
: ani :: CoEmitter IO ChartOptions

#+begin_src haskell-ng :results output
:t glue (Committer display) <$|> ani
#+end_src

#+RESULTS:
: glue (Committer display) <$|> ani :: IO ()

#+begin_src haskell-ng :results output
:t gapEffect <$> fmap (0.1,) <$> ani
#+end_src

#+RESULTS:
: gapEffect <$> fmap (0.1,) <$> ani
:   :: Codensity IO (Emitter IO ChartOptions)

#+begin_src haskell-ng :results output
grain = grid OuterPos (Range 0 1) 100 :: [Double]
gaps' x = glue (Committer display) <$> gapEffect <$> fmap (x,)
ani grain f = grid OuterPos (Range 0 1) grain & fmap (\x -> Lens.transformOnOf Lens.template Lens.uniplate (f x) lineExample) & qList
go = gaps' 0.1 <$|> ani 100 (\x -> over chroma' (*x))
-- ani f x = grid OuterPos (Range 0 1) x & (\x -> Lens.transformOnOf Lens.template Lens.uniplate (f x) lineExample) & qList
-- <$> (qList $ fmap (\x -> Lens.transformOnOf Lens.template Lens.uniplate (over chroma' (*1)) >>> over opac' (const x) lineExample) grain)
-- gaps' 0.2 <$|> ani (\x -> over chroma' (*x)) 100
#+end_src

#+RESULTS:
#+begin_example
<interactive>:41:11: error: [GHC-88464]
    Variable not in scope: glue :: t0 -> a0 -> b
    Suggested fix:
      Perhaps use data constructor ‘Blue’ (imported from Text.Pretty.Simple)

<interactive>:41:17: error: [GHC-88464]
    Data constructor not in scope:
      Committer :: (ChartOptions -> IO Bool) -> t0

<interactive>:41:40: error: [GHC-88464]
    Variable not in scope: gapEffect :: f (t, a) -> a0
<interactive>:42:61: error: [GHC-76037]
    Not in scope: ‘Lens.transformOnOf’
    Note: No module named ‘Lens’ is imported.

<interactive>:42:80: error: [GHC-76037]
    Not in scope: ‘Lens.template’
    Note: No module named ‘Lens’ is imported.

<interactive>:42:94: error: [GHC-76037]
    Not in scope: ‘Lens.uniplate’
    Note: No module named ‘Lens’ is imported.
<interactive>:43:6: error: [GHC-88464]
    Variable not in scope: gaps' :: t1 -> t2

<interactive>:43:16: error: [GHC-88464]
...
<interactive>:43:21: error: [GHC-88464]
    Variable not in scope:
      ani :: t0 -> (Double -> Colour -> Colour) -> t3
    Suggested fix:
      Perhaps use one of these:
        ‘and’ (imported from NumHask.Prelude),
        ‘any’ (imported from NumHask.Prelude),
        ‘T.any’ (imported from Data.Text)
#+end_example
...
c $ mempty @ChartOptions & (set #chartTree (legendFrame l l'))
...
lss = (\x -> defaultLineStyle & set #borderSize 0 & set #color (paletteO x 0.6) & set #size (0.025 * fromIntegral (x + 1))) <$> [0,1,2]
cs = zipWith (\l ps -> LineChart l [ps]) lss ls
lo = defaultLegendOptions & set #scaleP ScalePX & set #place PlaceRight & set #legendCharts (zipWith (\t c -> (t, [c])) ["palette #0", "palette #1", "palette #2"] cs) & set #numStacks 1 & set #scaleChartsBy 0.25 & set #stackAlign1 AlignLeft & set #stackAlign2 AlignMid & set #stackAlign3 AlignMid & set #alignCharts AlignRight
ho = defaultHudOptions & set #legends [ Priority 12 lo]
lineExample = mempty @ChartOptions & set #hudOptions ho & set #chartTree (named "line" cs)
debugLineExample = debugExample $ lineExample & Lens.transformOnOf Lens.template Lens.uniplate (over opac' (*0.7))

c debugLineExample
writeChartOptions "other/debuglineexample.svg" debugLineExample
#+end_src
...
#+RESULTS:
: cs!!2 :: Chart
: Just Rect 0.4775 0.5225 (-2.25e-2) 4.0225
: Chart {chartStyle = Style {size = 4.5e-2, borderSize = 1.0e-2, color = Colour 0.66 0.07 0.55 1.00, borderColor = Colour 0.02 0.29 0.48 1.00, scaleP = NoScaleP, textAnchor = AnchorMiddle, rotation = Nothing, translate = Nothing, escapeText = EscapeText, frame = Nothing, lineCap = Nothing, lineJoin = Nothing, dasharray = Nothing, dashoffset = Nothing, hsize = 0.6, vsize = 1.1, vshift = -0.25, glyphShape = SquareGlyph}, chartData = LineData [[Point 0.5 4.0,Point 0.5 0.0]]}
